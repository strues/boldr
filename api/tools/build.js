...
module.exports = task(
  ...
  ({ watch = false, onComplete } = {}) =>
    new Promise(resolve => {
      ...
      rimraf.sync('lib/*', { nosort: true, dot: true });

      let watcher = chokidar.watch(['src', '.boldr', 'package.json', 'yarn.lock']);
      watcher.on('all', (event, src) => {
        ...
        // Get destination file name, e.g. src/app.js (src) -> lib/app.js (dest)
        const dest = src.startsWith('src') ? `lib/${path.relative('src', src)}` : `lib/${src}`;
        ...
        try {
          switch (event) {
            ...
            // Create or update a file inside the output (lib) folder
            case 'add':
            case 'change':
              if (src.startsWith('src') && src.endsWith('.js')) {
                const { code, map } = babel.transformFileSync(src, {
                  sourceMaps: true,
                  sourceFileName: path.relative('./lib', src),
                });
                // Enable source maps
                const data =
