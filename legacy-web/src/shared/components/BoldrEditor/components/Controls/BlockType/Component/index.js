...
class LayoutComponent extends Component {
  ...
  renderFlat(blocks: Array<Object>): void {
    ...
    return (
      <div className={classNames('boldr-editor__inline-wrapper', className)}>
        {blocks.map(block => (
          <Option
            key={block.id}
            value={block.label}
  ...
  renderInDropdown(blocks: Array<Object>): void {
    ...
    const currentBlockData = this.blocksTypes.filter(
    ...
    return (
      <div
        className="boldr-editor__block-wrapper"
        aria-label="boldr-editor__block-control"
      >
        <Dropdown
          className={classNames('boldr-editor__block-dropdown', className)}
  ...
  render(): void {
    ...
    const blocks = this.blocksTypes.filter(({ label }) =>
      config.options.includes(label),
    );
