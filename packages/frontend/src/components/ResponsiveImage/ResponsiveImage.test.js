...
describe('(React Component) ResponsiveImage', () => {
 ...
 attraction in Rome"
  ...
  it('should initialize with a state of isResponsiveImageLoaded: false.', () => {
    const wrapper = shallow(
      <ResponsiveImage
        src="http://i.magaimg.net/img/frn.jpg"
    ...
    expect(wrapper.state('isResponsiveImageLoaded')).toEqual(undefined);
  });
  ...
  it('should respect the lazySrc prop when passed.', () => {
    ...
    expect(img.prop('src')).toEqual('http://i.magaimg.net/img/frn.jpg?h=10&w=14');
  });
  ...
  it('should render a <VisibilitySensor/> which will call the contexts handleViewportVisibilityChange() method.', () => {
    ...
    expect(sensor.prop('onChange')).toEqual(wrapper.instance().handleViewportVisibilityChange);
  });
  ...
  it('should render a <picutre/> and <source/> for each query if the isInViewport is truthy.', () => {
    ...
    expect(sources.at(0).prop('media')).toEqual('(min-width: 600px) AND (max-width: 899px)');
  });
  ...
  // it('handleResponsiveImageLoaded() should set the isResponsiveImageLoaded state to true and call the onLoad prop if provided.', () => {
  ...
  //   wrapper.instance().handleResponsiveImageLoaded();

  //   expect(wrapper.state('isResponsiveImageLoaded')).toEqual(true);
