            ...
            {images.length > 1 ? (
              <div
                className={`${prefix}-image-p-footer image-p-footer-paging ${showRotateBtn
                  ? 'show-rotate-btn'
                  : ''}`}>
                <span className={`${prefix}-image-p-action`} onClick={this.handlePreviousAction}>
                  previous
                </span>
                {showRotateBtn && (
                  <span className={`${prefix}-image-p-action`} onClick={this.handleRotate}>
                    rotate
                  </span>
                )}
                <span className={`${prefix}-image-p-action`} onClick={this.handleNextAction}>
                  next
                </span>
              </div>
            ) : (
              <div className={`${prefix}-image-p-footer ${showRotateBtn ? 'show-rotate-btn' : ''}`}>
                {showRotateBtn && (
                  <span
