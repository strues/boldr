...
function getSuggestionComponent() {
  ...
  return class SuggestionComponent extends Component {
    ...
    render() {
      ...
      return (
        <span
          className="boldr-editor__suggestion-wrapper"
          ref={this.setSuggestionReference}
          onClick={config.modalHandler.onSuggestionClick}
          aria-haspopup="true"
          aria-label="boldr-editor__suggestion-popup"
        >
          <span>
            {children}
          </span>
          {showSuggestions &&
            <span
              className={classNames('boldr-editor__suggestion-dropdown', dropdownClassName)}
            ...
            >
              {this.filteredSuggestions.map((suggestion, index) =>
                <span
                  ...
                  className={classNames('boldr-editor__suggestion-option', optionClassName, {
                    'boldr-editor__suggestion-option-active': index === activeOption,
                  })}
                >
                  {suggestion.text}
                </span>,
