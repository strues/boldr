...
const usesDevVendorDLL = bundleConfig =>
  bundleConfig.devVendorDLL != null && bundleConfig.devVendorDLL.enabled; // eslint-disable-line

const vendorDLLsFailed = (err) => {
  log({
    title: 'vendorDLL',
    level: 'error',
    message: 'DLLs ran into problems compiling. Check console for details.',
    notify: true,
  });
  if (err) {
    console.error(err);
