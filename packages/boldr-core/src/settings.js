...
export function attachRef(state, obj) {
  ...
  if (props._ref) {
    const ref = state.boldr.settings.ref[props._ref];
    invariant(ref, `Boldr/Settings: Reference '${props._ref}' not found`);
    props = Object.assign({}, state.boldr.settings.ref[props._ref], obj);
    delete props._ref;
  }
  return props;
}
...
export function mapStateToViewProps(state, ownProps) {
  let viewProps = {};
  if (ownProps.view) {
    viewProps = Object.assign({}, state.boldr.settings.views[ownProps.view]);
