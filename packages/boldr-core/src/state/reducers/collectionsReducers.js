/**
 * @module boldr-core/lib/state/reducers/collectionsReducers
 */
...
function deleteCollectionElement(state, action) {
  if (action.operations.delete) {
    ...
    throw new Error('BOLDR_CORE collection deletion is only compatible with ImmutableJs List and Map');
  }
  return state;
}
...
function updateCollectionElement(state, action) {
  if (action.operations.update) {
    ...
    throw new Error('BOLDR_CORE collection update is only compatible with ImmutableJs List and Map');
  }
  return state;
}
