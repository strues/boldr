...
const BOLDR_PROPS = [
  // componentDidMount action creator id in registry
  'didMountActionCreator',
  // redux state management on unmount
  'keepComponentState',
  // view component id in registry
  'view',
  // componentWillUnmount action creator id in registry
  'willUnMountActionCreator',
];
...
export function getStateToProps({
  ...
  const boldrProps = getStateProps(
    state,
    getComponentName(WrappedComponent),
    getComponentId(componentId, ownProps),
  );
  boldrProps.getCollection = function getCollection(id) {
    return state.boldr.collections.get(id);
  };
  ...
  let userProps = {};
  if (mapStateToProps) {
    userProps = mapStateToProps(state, ownProps, boldrProps);
  }

  return { ...boldrProps, ...viewProps, ...userProps };
}
...
export function getDispatchToProps({
  ...
  const boldrProps = getStateAccessors(
    dispatch,
    getComponentName(WrappedComponent),
    getComponentId(componentId, ownProps),
    defaultState,
  );
  boldrProps.dispatch = dispatch;
  boldrProps.dispatchActionCreator = (actionId, event, data, context) => {
    dispatch(api.action.getActionCreatorFunction(context, actionId)(event, data, context));
  };

  let userProps = {};
  if (mapDispatchToProps) {
    userProps = mapDispatchToProps(dispatch, ownProps, boldrProps);
  }

  return { ...boldrProps, ...userProps };
}
 ...
 */
export default function boldrConnect({
  componentId,
  defaultState,
  keepComponentState,
  mapStateToProps,
  ...
  return function wrapWithBoldr(WrappedComponent) {
    class BoldrContainer extends React.Component {
      static displayName = `BOLDR(${WrappedComponent.displayName})`;
      static propTypes = {
    ...
    const Connected = connect(
      ...
      mergeProps,
      { ...rest },
    )(hoistStatics(BoldrContainer, WrappedComponent));
    Connected.BoldrContainer = BoldrContainer;
    return Connected;
  };
}
