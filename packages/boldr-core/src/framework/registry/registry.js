...
function addToRegistry(id, item) {
  if (Registry.isLocked()) {
    throw new Error(
      `BOLDR_CORE: The registry is locked, you cannot therefore add '${id}' in it. ` +
        'Please check your BOLDR_CORE configuration, it should not move after the initial ' +
  ...
  if (registry[id]) {
    console.warn(
      ...
      `BOLDR_CORE: The '${id}' object is registered, overriding an existing '${id}' object. ` +
        'Please check your BOLDR_CORE configuration, you might not want that.',
    );
  }
  if (item === undefined) {
    throw new Error(
