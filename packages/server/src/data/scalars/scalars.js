...
const complexityValidator = function(ast, options) {
  ...
  if (complexity.alphaNumeric && !alhpaNumericRe.test(ast.value)) {
    throw new GraphQLError('Query error: String must contain at least one number and one letter', [
      ast,
    ]);
  }
  ...
  if (complexity.specialChars && !specialCharsRe.test(ast.value)) {
    throw new GraphQLError('Query error: String must contain at least one special character', [
      ast,
    ]);
  }
};
