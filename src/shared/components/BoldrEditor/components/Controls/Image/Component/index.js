...
class LayoutComponent extends Component {
  ...
  state: Object = {
    imgSrc: '',
    dragEnter: false,
    uploadHighlighted: this.props.config.uploadEnabled &&
      !!this.props.config.uploadCallback,
  ...
  componentWillReceiveProps(props: Object): void {
    if (this.props.expanded && !props.expanded) {
      ...
      this.setState({
        uploadHighlighted: props.config.uploadEnabled &&
          !!props.config.uploadCallback,
      });
    }
  }
  ...
  renderAddImageModal(): Object {
    ...
    return (
      <div
        className={classNames('boldr-editor__image-modal', popupClassName)}
        onClick={this.stopPropagation}
      >
        <div className="boldr-editor__image-modal-header">
          {uploadEnabled &&
            uploadCallback &&
            <span
              onClick={this.showImageUploadOption}
              className="boldr-editor__image-modal-header-option"
            >
              Upload
              <span
                className={classNames(
                  'boldr-editor__image-modal-header-label',
                  {
                    'boldr-editor__image-modal-header-label-highlighted': uploadHighlighted,
                  },
                )}
              />
            </span>}
        ...
        {uploadHighlighted
          ? <div onClick={this.fileUploadClick}>
              <div
                ...
                className={classNames(
                  'boldr-editor__image-modal-upload-option',
                  {
                    'boldr-editor__image-modal-upload-option-highlighted': dragEnter,
                  },
                )}
              >
                <label
                  htmlFor="file"
                  className="boldr-editor__image-modal-upload-option-label"
                >
              ...
              <input
                ...
                className="boldr-editor__image-modal-upload-option-input"
              />
            </div>
          : <div className="boldr-editor__image-modal-url-section">
              <input
                className="boldr-editor__image-modal-url-input"
        ...
        <span className="boldr-editor__image-modal-btn-section">
          <button
            className="boldr-editor__image-modal-btn"
        ...
        {showImageLoading
          ? <div className="boldr-editor__image-modal-spinner">
              <Spinner />
            </div>
          : undefined}
      </div>
  ...
  render(): Object {
    ...
    return (
      <div
        className="boldr-editor__image-wrapper"
        aria-haspopup="true"
        aria-expanded={expanded}
        aria-label="boldr-editor__image-control"
      >
        <Option
