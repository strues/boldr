...
Object.keys(boldrConfig.cspExtensions).forEach((key) => {
  if (cspConfig.directives[key]) {
    cspConfig.directives[key] = cspConfig.directives[key]
      .concat(boldrConfig.cspExtensions[key]);
  } else {
    cspConfig.directives[key] = boldrConfig.cspExtensions[key];
  }
});
...
if (process.env.NODE_ENV === 'development') {
  ...
  Object.keys(cspConfig.directives).forEach((directive) => {
    cspConfig.directives[directive].push(
      `${envConfig.host}:${envConfig.wpdsPort}`,
    );
  });
}
