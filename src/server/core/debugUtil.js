...
export function cleanSourceFileName(sourceFile) {
  let cleanFile = sourceFile.split(':')[1] || sourceFile;
...
export function frameToString(frame) {
  ...
  let sourceFile = wrappedFrame.getFileName();
  let functionName = wrappedFrame.getFunctionName();
  let typeName = wrappedFrame.getTypeName();
  ...
  const identifier = functionName || typeName || '<anonymous>';
