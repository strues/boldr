...
export default class LayoutComponent extends Component {
  ...
  renderInFlatList(): Object {
    ...
    return (
      <div
        className={classNames('boldr-editor__list-wrapper', className)}
        aria-label="boldr-editor__list-control"
      >
  ...
  renderInDropDown(): Object {
    ...
    return (
      <Dropdown
        className={classNames('boldr-editor__list-dropdown', className)}
      ...
      >
        ...
        {this.options.filter(option => options.indexOf(option) >= 0).map((option, index) =>
          <DropdownOption
            key={index}
            value={option}
            className={classNames('boldr-editor__list-dropdownOption', config[option].className)}
