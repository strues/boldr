...
class LayoutComponent extends Component {
  ...
  renderAddLinkModal() {
    ...
    return (
      <div
        className={classNames('boldr-editor__link-modal', popupClassName)}
        onClick={stopPropagation}
      >
        <span className="boldr-editor__link-modal-label">
          Link Title
        </span>
        <input
          className="boldr-editor__link-modal-input"
        ...
        <input
          ...
          value={linkTarget}
        />
        <span className="boldr-editor__link-modal-target-option">
          <input
            type="checkbox"
        ...
        <span className="boldr-editor__link-modal-buttonsection">
          <button
            className="boldr-editor__link-modal-btn"
            onClick={this.addLink}
            disabled={!linkTarget || !linkTitle}
          >
            Add
          </button>
          <button className="boldr-editor__link-modal-btn" onClick={doCollapse}>
            Cancel
          </button>
        </span>
      </div>
  ...
  renderInFlatList(): Object {
    ...
    return (
      <div
        className={classNames('boldr-editor__link-wrapper', className)}
        aria-label="boldr-editor__link-control"
      >
  ...
  renderInDropDown(): Object {
    ...
    return (
      <div
        className="boldr-editor__link-wrapper"
        aria-haspopup="true"
        aria-label="boldr-editor__link-control"
      ...
      >
        <Dropdown
          className={classNames('boldr-editor__link-dropdown', className)}
        ...
        >
          ...
          {options.indexOf('link') >= 0 &&
            <DropdownOption
              ...
              className={classNames('boldr-editor__link-dropdownoption', link.className)}
              title={link.title}
            >
              <Icon kind="link" color="#222" />
            </DropdownOption>}
          {options.indexOf('unlink') >= 0 &&
            <DropdownOption
              onClick={this.removeLink}
              disabled={!currentState.link}
              className={classNames('boldr-editor__link-dropdownoption', unlink.className)}
              title={unlink.title}
            >
              <Icon kind="unlink" color="#222" />
            </DropdownOption>}
