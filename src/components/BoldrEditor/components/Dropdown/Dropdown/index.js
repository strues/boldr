...
export default class Dropdown extends Component {
  ...
  render() {
    ...
    return (
      <div
        className={classNames('boldr-editor__dropdown-wrapper', className)}
        aria-expanded={expanded}
        aria-label={ariaLabel || 'boldr-editor__dropdown'}
      >
        <a className="boldr-editor__dropdown-selectedtext" onClick={onExpandEvent} title={title}>
          {children[0]}
          <div
            className={classNames({
              'boldr-editor__dropdown-carettoclose': expanded,
              'boldr-editor__dropdown-carettoopen': !expanded,
            })}
          />
        </a>
        {expanded
          ? <ul
              className={classNames('boldr-editor__dropdown-optionwrapper', optionWrapperClassName)}
