...
describe('Controlled CheckboxGroup', () => {
  it('value', () => {
    ...
    const radios = TestUtils.scryRenderedComponentsWithType(radioGroup, Checkbox);

    const appleCheckboxNode = ReactDOM.findDOMNode(radios[0]);
    const bananaCheckboxNode = ReactDOM.findDOMNode(radios[1]);

    const appleCheckboxInputNode = TestUtils.findRenderedDOMComponentWithTag(radios[0], 'input');
    const bananaCheckboxInputNode = TestUtils.findRenderedDOMComponentWithTag(radios[1], 'input');

    expect(appleCheckboxNode.className).toContain('boldrui-checkbox-checked');
    expect(appleCheckboxInputNode.checked).toBe(true);

    expect(bananaCheckboxNode.className).not.toContain('boldrui-checkbox-checked');
  ...
  describe('disabled', () => {
    it('disabled group', () => {
      ...
      const checkboxs = TestUtils.scryRenderedComponentsWithType(checkboxGroup, Checkbox);
      ...
      expect(appleCheckboxNode.className).toContain('boldrui-checkbox-disabled');
      expect(bananaCheckboxNode.className).toContain('boldrui-checkbox-disabled');
    ...
    it('checkbox disabled in group', () => {
      ...
      expect(appleCheckboxNode.className).toContain('boldrui-checkbox-disabled');
      expect(bananaCheckboxNode.className).not.toContain('boldrui-checkbox-disabled');
    ...
    it('checkbox not disabled in disabled group', () => {
      ...
      expect(appleCheckboxNode.className).not.toContain('boldrui-checkbox-disabled');
      expect(bananaCheckboxNode.className).toContain('boldrui-checkbox-disabled');
  ...
  it('className', () => {
    const radioGroup = TestUtils.renderIntoDocument(<CheckboxGroup className="customClassName" />);
