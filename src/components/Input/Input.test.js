...
describe('Input', () => {
  ...
  it('can have custom prefix of classNames', () => {
    ...
    expect(wrapper.find('span').at(0).hasClass('foo-input__addon-before')).toBe(true);
    expect(wrapper.find('input').hasClass('foo-input')).toBe(true);
    expect(wrapper.find('span').at(1).hasClass('foo-input__addon-after')).toBe(true);
  });
  ...
  it('pass any props to real input element except "className" & "prefix"', () => {
    ...
    expect(wrapper.find('input').props().className).toBe('boldrui-input');
  ...
  it('can distinguish enter and other keys through keyDown event', () => {
    ...
    const wrapper = shallow(
      <Input onKeyUp={onKeyUpMock} onKeyDown={onKeyDownMock} onPressEnter={onPressEnterMock} />,
    );
