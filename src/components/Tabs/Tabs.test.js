...
describe('Tabs', () => {
  ...
  it('custom className and prefix', () => {
    const wrapper = mount(
      <Tabs activeId="1" className="foobar-cls" prefix="duck">
        <TabPanel id="1" tab="tab-title">
          foobar
        </TabPanel>
      </Tabs>,
    );
    expect(wrapper.find('.duck-tabs.foobar-cls').length).toBe(1);
  });
    ...
    const ensure = align => {
      const wrapper = mount(
        <Tabs activeId="foobar" align={align}>
          ...
          <TabPanel id="duck" tab="duck-tab">
            duck
          </TabPanel>
        </Tabs>,
      );
      align = align || 'left';
      expect(wrapper.find('.boldrui-tabs-nav').hasClass(`boldrui-tabs-align-${align}`)).toBe(true);
    };
    ensure();
    ensure('left');
    ensure('right');
  ...
  it('different sizes', () => {
    const ensure = size => {
      ...
      size = size || 'normal';
      expect(wrapper.find('.boldrui-tabs-nav').hasClass(`boldrui-tabs-size-${size}`)).toBe(true);
    };
    ensure();
    ensure('normal');
    ensure('huge');
  ...
  it('different types', () => {
    const ensure = type => {
      ...
      type = type || 'normal';
      expect(wrapper.find('.boldrui-tabs-nav').hasClass(`boldrui-tabs-type-${type}`)).toBe(true);
    };
    ensure();
    ensure('normal');
    ensure('card');
  ...
  it('onTabChange callback', () => {
    ...
    expect(onChange.mock.calls[0][0]).toBe('duck');
  });
  ...
  it('onTabDel and onTabAdd callback', () => {
    ...
    wrapper.find('.boldrui-tabs-nav-add').simulate('click');
    expect(wrapper.find('Tab').length).toBe(3);

    wrapper.find('Tab').last().simulate('click');
    expect(wrapper.state('active')).toBe('bar');

    wrapper.find('Tab').first().find('.boldrui-tabs-tab-inner-del').simulate('click');
    expect(wrapper.find('Tab').length).toBe(2);
    expect(wrapper.state('tabs')).toEqual(['duck', 'bar']);
  });
  ...
  it('can render withour any panel', () => {
    expect(() => mount(<Tabs activeId="1" className="foobar-cls" prefix="duck" />)).not.toThrow();
  });
});
