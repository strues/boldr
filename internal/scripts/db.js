...
const config = {
  ...
  migrations: {
    ...
    directory: path.resolve(process.cwd(), './db/_migrations'),
  },
  seeds: {
    directory: path.resolve(process.cwd(), './db/_seeds'),
  },
};
  ...
  try {
    switch (command) {
      case 'version':
        ...
        break;
      case 'migration':
        fs.writeFileSync(`db/_migrations/${version}_${process.argv[3] || 'new'}.js`, template, 'utf8');
        break;
      case 'migrate:undo':
        db = knex(config);
